...
contract data {
  ...
  event LogNewUser   (address indexed userAddress, uint index, string userReferral, uint userWeightage,uint userAmount);
  event LogUpdateUser(address indexed userAddress, uint index, string userReferral, uint userWeightage , uint userAmount);
  ...
  function insertUser(address userAddress, string userReferral, uint userWeightage,uint userAmount) validOwner public returns(uint index){
      ...
      emit LogNewUser(userAddress, userStructs[userAddress].index, userReferral, userWeightage,userAmount);
  ...
  function deleteUser(address userAddress) validOwner  public returns(uint index) {
      ...
      emit LogUpdateUser(keyToMove, rowToDelete, userStructs[keyToMove].userReferral, userStructs[keyToMove].userWeightage, userStructs[keyToMove].userAmount);
      return rowToDelete;
  }
  
  function getUser(address userAddress)public  constant returns(string userReferral, uint userWeightage,uint userAmount, uint index){
    if(!isUser(userAddress))  revert(); 
    return(
  ...
  function updateUserReferral(address userAddress, string userReferral) validOwner  public returns(bool success) {
    if(!isUser(userAddress))  revert(); 
    userStructs[userAddress].userReferral = userReferral;
      emit  LogUpdateUser(userAddress, userStructs[userAddress].index, userReferral, userStructs[userAddress].userWeightage,  userStructs[userAddress].userAmount);
      return true;
  }
  ...
  function updateUserWeightage(address userAddress, uint userWeightage) validOwner public returns(bool success) 
  {
    if(!isUser(userAddress))  revert(); 
    userStructs[userAddress].userWeightage = userWeightage;
  emit LogUpdateUser(userAddress, userStructs[userAddress].index,userStructs[userAddress].userReferral, userWeightage, userStructs[userAddress].userAmount);
    return true;
  }
    ...
    if(!isUser(userAddress))  revert(); 
    userStructs[userAddress].userAmount += userAmount ;
    emit LogUpdateUser(userAddress, userStructs[userAddress].index,userStructs[userAddress].userReferral, userAmount, userStructs[userAddress].userWeightage);
    return true;
  }
